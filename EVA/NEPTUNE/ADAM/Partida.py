...
class Partida: 
    ...
    def posible_mov(self, desX, desY, color_a_mover, comer=False, validarMas=True):
        #Vamos a coger una copia para simular una jugada en ella
        partida_copia = deepcopy(self)
        partida_copia.mueveBlancas = (True if color_a_mover == "B" else False)
        ...
        for i in ["T", "C", "A", "D", "R"]:
            if partida_copia.ejecutar_jugada(f"{i}x{chr(desX+96)}{desY}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{i}x{chr(desX+96)}{desY}")
            
            if partida_copia.ejecutar_jugada(f"{i}{chr(desX+96)}{desY}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{i}{chr(desX+96)}{desY}")
            
            if partida_copia.ejecutar_jugada(f"{i}{chr(desX+96)}x{chr(desX+96)}{desY}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{i}{chr(desX+96)}x{chr(desX+96)}{desY}")

            if partida_copia.ejecutar_jugada(f"{i}{chr(desX+96)}{chr(desX+96)}{desY}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{i}{chr(desX+96)}{chr(desX+96)}{desY}")
        
            if partida_copia.ejecutar_jugada(f"{i}{desY}x{chr(desX+96)}{desY}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{i}{desY}x{chr(desX+96)}{desY}")
            
            if partida_copia.ejecutar_jugada(f"{i}{desY}{chr(desX+96)}{desY}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{i}{desY}{chr(desX+96)}{desY}")

            if partida_copia.ejecutar_jugada(f"{chr(desX+95)}x{chr(desX+96)}{desY}={i}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{chr(desX+95)}x{chr(desX+96)}{desY}={i}")

            if partida_copia.ejecutar_jugada(f"{chr(desX+97)}x{chr(desX+96)}{desY}={i}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{chr(desX+97)}x{chr(desX+96)}{desY}={i}")

            if partida_copia.ejecutar_jugada(f"{chr(desX+96)}{desY}={i}", False, validarMas) >= 0:
                posibles_movimientos.append(f"{chr(desX+96)}{desY}={i}")

        if partida_copia.ejecutar_jugada(f"{chr(desX+95)}x{chr(desX+96)}{desY}", False, validarMas) >= 0:
            posibles_movimientos.append(f"{chr(desX+95)}x{chr(desX+96)}{desY}")

        if partida_copia.ejecutar_jugada(f"{chr(desX+97)}x{chr(desX+96)}{desY}", False, validarMas) >= 0:
            posibles_movimientos.append(f"{chr(desX+97)}x{chr(desX+96)}{desY}")

        if partida_copia.ejecutar_jugada(f"{chr(desX+96)}{desY}", False, validarMas) >= 0:
            posibles_movimientos.append(f"{chr(desX+96)}{desY}")
        
        if partida_copia.ejecutar_jugada("O-O-O", False, validarMas) >= 0:
            posibles_movimientos.append("O-O-O")
        
        if partida_copia.ejecutar_jugada("O-O", False, validarMas) >= 0:
            posibles_movimientos.append("O-O")
            
        return posibles_movimientos

    def posibles_movimientos(self, color):
        lista_mov = []
        ...
        for i in range(1, 9):
            for j in range(1, 9):
                destino = self.comprueba_pieza_casilla(i, j)
                if (destino is None and self.hay_mov(i, j, color, comer=False)):
                    lista_mov.extend(self.posible_mov(i, j, color, comer=False))

                if (destino is not None and destino.nombre[-1] == ("B" if color == "N" else "N") and self.hay_mov(i, j, color, comer=True)):
                    lista_mov.extend(self.posible_mov(i, j, color, comer=True))
