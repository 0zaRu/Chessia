...
from LILITH.lilith03 import Lilith_Predictor as Lilith
import Menu
import pygame

global coronando, pieza, jugada_promocion, alertas, finalizado, jugadas, partidas
alertas = W.CuadroTexto("", 750, 40, 300, 40, W.AZUL_FONDO, W.AZUL_FONDO, W.AZUL_OSCURO, 14)
coronando = finalizado = False
pieza = jugada_promocion = ""
jugadas = []

# Cargar el archivo de partidas
with open(W.RUTA_DATOS, "r") as f:
    partidas = f.readlines()
...
def nuevo_movimiento(partida, color="B"):
    ...
    if color == "B":
        ...
        posibles_mov = partida.posibles_movimientos("N")

        mejores_mov = Lilith.predecir_mejores_10(jugadas, posibles_mov, partidas)
        mov_ejecutado = ""
        for mov in mejores_mov:
            resultado = partida.ejecutar_jugada(mov)
            if resultado >= 0: 
                mov_ejecutado = mov
                break

        interpreta_resultado(partida, resultado, mov_ejecutado)
...
def dibujar(ventana):
    ...
    menu_icon = pygame.image.load(W.RUTA_ORIGEN+"source/LILITH icon.png")  
    pygame.display.set_icon(menu_icon)
    pygame.display.set_caption("Juego con LILITH")

    volver_boton = W.Boton("←", 10, 10, 40, 40, W.AZUL_OSCURO, W.AZUL_CLARO, 40, Menu.dibujar)
    ...
    cuadro_jugadas =       W.CuadroTexto("",             750, 355, 300, 345,W.AZUL_CLARO, W.AZUL_OSCURO, W.AZUL_OSCURO, 18)
    salir_boton =          W.Boton("Salir",              900, 750, 180, 40, W.NEGRO, W.BLANCO, 21, W.salir)
    text_botonB =          W.Boton("Turno Blanco",       143, 685, 540, 80, W.AZUL_FONDO, W.NEGRO, 26, W.salir, center_text=False)
    text_botonN =          W.Boton("Turno Negro",        262,  -2, 300, 80, W.AZUL_FONDO, W.NEGRO, 26, W.salir, center_text=False)
    nombre_lilith =        W.Boton("LILITH03",           200, 17, 100, 80, W.AZUL_FONDO, W.NEGRO, 26, W.salir)

    menu_seleccion =   W.CuadroTexto("", 205, 335, 375, 120, W.AZUL_CLARO, W.AZUL_OSCURO, W.AZUL_OSCURO, 18)
    menu_seleccion.cambiar_texto(" SELECCIONA UNA PARA PROMOCIONAR")
    dama_boton =       W.Boton_img(W.RUTA_ORIGEN + "source/piezas/dama-blanco.png",    253, 370, 68, 68, W.AZUL_CLARO, W.AZUL_OSCURO, select_pieza_promo)
    caballo_boton =    W.Boton_img(W.RUTA_ORIGEN + "source/piezas/caballo-blanco.png", 328, 370, 68, 68, W.AZUL_CLARO, W.AZUL_OSCURO, select_pieza_promo)
    alfil_boton =      W.Boton_img(W.RUTA_ORIGEN + "source/piezas/alfil-blanco.png",   403, 370, 68, 68, W.AZUL_CLARO, W.AZUL_OSCURO, select_pieza_promo)
    torre_boton =      W.Boton_img(W.RUTA_ORIGEN + "source/piezas/torre-blanco.png",   478, 370, 68, 68, W.AZUL_CLARO, W.AZUL_OSCURO, select_pieza_promo)

    while True:
        ventana.fill(W.AZUL_FONDO)
        ...
        nombre_lilith.dibujar(ventana)
        logo = pygame.image.load(W.RUTA_ORIGEN+"source/LILITH icon.png")  
        logo = pygame.transform.scale(logo, (60, 60)) 
        ventana.blit(logo, (120, 26))
        
        mouse_pos = pygame.mouse.get_pos()

        W.dibujar_tablero(ventana, partida.tablero, W.AZUL_CLARO, W.AZUL_OSCURO, "B", pieza_seleccionada)
        if coronando:
